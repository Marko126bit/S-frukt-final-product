...
#log in system/////////////////////////////////////////////////////////////////////
   ...
   #logic for small and partner/////////////////////////////////////////////////
    ...
    # Get search and filter parameters
     ...
     # Base query for logs
     logs_query = TransactionLog.query.order_by(TransactionLog.timestamp.desc())

     # Apply search and filter criteria
     if search_query:
        logs_query = logs_query.filter(
            (TransactionLog.source_id.in_(partners.keys()) & (TransactionLog.source_type == 'partner') & (Partner.name.ilike(f'%{search_query}%'))) |
            (TransactionLog.destination_id.in_(partners.keys()) & (TransactionLog.destination_type == 'partner') & (Partner.name.ilike(f'%{search_query}%'))) |
            (TransactionLog.source_id.in_(small_storages.keys()) & (TransactionLog.source_type == 'small_storage') & (SmallStorage.name.ilike(f'%{search_query}%'))) |
            (TransactionLog.destination_id.in_(small_storages.keys()) & (TransactionLog.destination_type == 'small_storage') & (SmallStorage.name.ilike(f'%{search_query}%')))
        )

     if filter_type == 'partners':
        logs_query = logs_query.filter(
            (TransactionLog.source_type == 'partner') | (TransactionLog.destination_type == 'partner')
        )
     elif filter_type == 'small_storages':
        logs_query = logs_query.filter(
            (TransactionLog.source_type == 'small_storage') | (TransactionLog.destination_type == 'small_storage')
        )
     ...
     for log in logs:
        ...
        if log.destination_type == 'partner' and log.destination_id in partners:
            ...
            log.destination_name = 'Skladište'
            
        # Convert timestamp to local time zone
        log.local_timestamp = log.timestamp.replace(tzinfo=pytz.utc).astimezone(local_tz)

     return render_template('transaction_logs.html', logs=logs, search_query=search_query, filter_type=filter_type)
...
#EXCEL IZVESTAJ///////////////////////////////////////
    ...
    @app.route('/export_transaction_logs_to_excel')
    def export_transaction_logs_to_excel():
        # Get search and filter parameters
        search_query = request.args.get('search', '')
        filter_type = request.args.get('filter', '')
        ...
        for log in logs:
            ...
            source_name = partners.get(log.source_id, small_storages.get(log.source_id, 'S-frukt'))
            destination_name = partners.get(log.destination_id, small_storages.get(log.destination_id, 'Skladište'))

            # Define action type in Serbian
            action_type = 'Dodato' if log.action_type == 'add' else 'Oduzeto'

            # Convert timestamp to local time zone
            local_tz = pytz.timezone('Europe/Berlin')
            local_timestamp = log.timestamp.replace(tzinfo=pytz.utc).astimezone(local_tz)

            data.append({
                'ID': log.id,
                'Akcija': action_type,
                'Izvor': source_name,
                'Odredište': destination_name,
                'Količina': log.quantity,
                'Vreme': local_timestamp.strftime('%Y-%m-%d %H:%M:%S')
            })
        ...
        # Get the current date and time in Serbian format
        local_time = datetime.now(local_tz)
        formatted_time = local_time.strftime('%d-%m-%Y_%H-%M-%S')

        # Send the Excel file as a response with a formatted filename
        return send_file(
            output,
            as_attachment=True,
            download_name=f'transakcije_{formatted_time}.xlsx',
