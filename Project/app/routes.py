...
#////////////////////edit delete add//////////////////////////////////////////////
    ...
    @app.route('/return_all_to_main', methods=['POST'])
    def return_all_to_main_storage():
    # Retrieve the single instance of main storage
     main_storage = MainStorage.query.get(1)  # Assuming you have only one instance of MainStorage
    ...
    # Sum quantities of all small storages
     total_returned_quantity = 0
     small_storages = SmallStorage.query.all()

     for storage in small_storages:
        total_returned_quantity += storage.quantity
        storage.quantity = 0  # Reset quantity for small storage

    # Add the summed quantity back to the main storage
     main_storage.quantity += total_returned_quantity

    # Commit the changes to the database
     db.session.commit()

     return jsonify({
        'success': 'All quantities returned to main storage',
        'new_main_storage_quantity': main_storage.quantity
    })
