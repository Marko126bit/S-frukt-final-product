...
from .models import MainStorage, Partner,  SmallStorage, TransactionLog, User
...
#log in system/////////////////////////////////////////////////////////////////////
   ...
   #logic for small and partner/////////////////////////////////////////////////
    ...
    def adjust_main_storage():
     ...
     main_storage = MainStorage.query.get(1)
     partner = Partner.query.get(partner_id)
     ...
     if operation == 'add':
        if partner.quantity >= quantity:
            ...
            # Ensure contribution_amount is initialized
            partner.contribution_amount = (partner.contribution_amount or 0) + quantity
            main_storage.quantity += quantity
        else:
        ...
        if main_storage.quantity >= quantity:
            main_storage.quantity -= quantity
            # Ensure contribution_amount won't go negative
            partner.contribution_amount = (partner.contribution_amount or 0) - quantity
            partner.quantity += quantity
        else:
     ...
     # Fetch and serialize updated partner data
     updated_partner_data = Partner.query.all()
     serialized_partner_data = [
        {
            'id': partner.id,
            'name': partner.name,
            'contribution': partner.contribution_amount or 0  # Ensure a default of 0 if None
        } for partner in updated_partner_data
     ]

     # Calculate the total contribution amount
     total_contribution_amount = sum(p['contribution'] for p in serialized_partner_data)
     ...
     return jsonify({
        ...
        'partner_contributions': serialized_partner_data
     })
    
    @app.route('/get_storage_data', methods=['GET'])
    def get_storage_data():
     main_storage = MainStorage.query.get(1)
     partners = Partner.query.all()

     partner_contributions = [
        {'name': partner.name, 'contribution': partner.contribution_amount or 0}
