...
class MainStorage(db.Model):
    ...
    def get_main_storage():
        storage = MainStorage.query.first()
        if not storage:
            storage = MainStorage()
    ...
    def adjust_quantity(self, amount):
        if amount < 0 and self.total_quantity < abs(amount):
            raise ValueError("Insufficient quantity in main storage.")
...
class MainStorageTransaction(db.Model):
    ...
    small_storage_id = db.Column(db.Integer, db.ForeignKey('storage.id'))  # Ensure small_storage table name is correct
