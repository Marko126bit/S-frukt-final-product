...
def update_partner_balance(partner_id, amount, is_deposit=False):
    partner = Partner.query.get(partner_id)
    if partner:
        # Negate the amount if it's a deposit to the main storage
        if is_deposit:
            amount = -abs(amount)
        partner.quantity += amount
        db.session.commit()
        return f"Balance updated successfully. New balance for {partner.name} is {partner.quantity}."
    else:
...
class TransactionLog(db.Model):
    ...
    transaction_logs = []
    destination_id = db.Column(db.Integer, db.ForeignKey('small_storage.id'))
    destination = db.relationship('SmallStorage', backref='transactions')
